...
/* Demonstrate use of two separate VBOs with different contents & attributes. 
	...
	09b)--In main(), replace the 'initShaders()' call with createProgram() calls 
		(both found in textbook's 'cuon-utils.js' library) to build the two shaders.
			...
			gl.program (e.g. args to getAttribLocation(), getUniformLocation(), etc) with  g_ShaderID1, and call 'useProgram(g_ShaderID1)' at the start of the 
...
var VSHADER_2SRC =										// VERTEX SHADER for VBO2: reddish tinged
  ...
  '		else {gl_PointSize = 1.0;}\n' +
  '  gl_Position = u_ModelMatrix * a_Position;\n' +
  '	 v_Colr = a_Color*0.6 + vec3(0.4, 0.0, 0.0);\n' + // ALL 40% red color!
  ' }\n';
...
var a_PositionLoc1;						// GPU location for 'a_Position' attrib in Shader 1
var a_PositionLoc2;						//   						... in Shader 2 (e.g. g_ShaderID2)
var a_ColorLoc1;							// GPU location for 'a_Color' attrib in shader 1
...
var u_ModelMatrixLoc1;				// GPU location for u_ModelMatrix uniform, Shader 1
...
// Version 9b):  MOVE THIS to initVBO1() and initVBO2()
 u_ModelMatrixLoc1 = gl.getUniformLocation(g_ShaderID1, 'u_ModelMatrix');
  if (!u_ModelMatrixLoc1) { 
  ...
  a_PositionLoc1 = gl.getAttribLocation(g_ShaderID1, 'a_Position');
  if(a_PositionLoc1 < 0) {
    console.log('initVBO1() Failed to get GPU storage location1 of a_Position');
    return -1;
  }
  ...
  gl.vertexAttribPointer(
		a_PositionLoc1,//index == ID# for the attribute var in your GLSL shaders;
  ...
  // Next attribute: a) get the GPU location...
 	a_ColorLoc1 = gl.getAttribLocation(g_ShaderID1, 'a_Color');
  if(a_ColorLoc1 < 0) {
    console.log('initVBO1() failed to get the GPU storage location1 of a_Color');
    return -1;
  }
  // b) tell GPU shader 1 how to retrieve attrib from the currently bound buffer:
  gl.vertexAttribPointer(a_ColorLoc1, 3, gl.FLOAT, false, 7*FSIZE1, 4*FSIZE1);
  // c) Enable this assignment of a_Color attribute to the bound buffer:
  gl.enableVertexAttribArray(a_ColorLoc1);
  //--------------------------------------------
  // Next attribute: a) get the GPU location...
...
//==============================================================================
  ...
  gl.clear(gl.COLOR_BUFFER_BIT);
	...
	if(g_show1 == 1) {	// IF user didn't press HTML button to 'hide' VBO1:
	  ...
	  gl.uniformMatrix4fv(u_ModelMatrixLoc1, 			// GPU location of the uniform
  	...
  	// currently-bound buffer:
		...
		//		http://www.opengl.org/sdk/docs/man/xhtml/glVertexAttribPointer.xml )
	  // ----------Tie shader's 'a_Position' attribute to bound buffer:------------
	  	  gl.vertexAttribPointer( a_PositionLoc1, 4, gl.FLOAT,false, 7*FSIZE1, 0);
