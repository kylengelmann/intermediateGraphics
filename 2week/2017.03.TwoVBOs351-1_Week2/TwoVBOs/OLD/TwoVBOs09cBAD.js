...
/* Demonstrate use of two separate VBOs with different contents & attributes. 
	...
	09b)--In main(), replace the 'initShaders()' call with createProgram() calls 
		...
		-----------------
	09c)--STOP sharing the same uniforms and attributes a_Position and a_Color 
			attribs for different VBOs: First, rename as a_Position1, a_Color1, test? Works! 
		  
		  
		  
		  Next, create a new 
		  set of attributes a_Position2, a_Color2 for use with VBO2.
		  ===================
		  
		   and separate attributes for VBO1 and VBO2,a_Position becomes a_Position1, a_Position2; Change g_ShaderID1 to g_ShaderID2 in initVBO2() function, so that everywhere we  draw(), add gl.useProgram() calls SWITCH between the two different shader programs as we draw each VBO's contents.  (Shader 2 makes VBO2 look purple)
		  
		  
		UNTANGLE the two different shader-programs and the two different VBOs.
			--For each VBO, create a SEPARATE, differently-named set of attributes.
			--For each shader, use ONLY the the attributes  for each shader, and assign it oncSTOP re-assigning the data-sources for those attributes -- the attribute
...
var u_ModelMatrix1Loc;					// GPU storage location of u_ModelMatrix1 uniform
...
// Create a buffer object in the graphics hardware: save its ID# in global var: 
 ...
 // 			times and then discarded.
  ...
  a_PtSizeLoc = gl.getAttribLocation(g_ShaderID1, 'a_PtSize');
  if(!a_PtSizeLoc) {
...
//	  gl.useProgram(g_ShaderID1);					// In the GPU, SELECT one of the vertex 
 ...
 // 			times and then discarded.
  ...
  a_Position1Loc = gl.getAttribLocation(g_ShaderID1, 'a_Position');
  if(a_Position1Loc < 0) {
