...
// Get GPU storage location for all uniform vars used by any of our shaders:
  u_ModelMatrixLoc = gl.getUniformLocation(g_ShaderID1, 'u_ModelMatrix');
  if (!u_ModelMatrixLoc) { 
    console.log('Failed to get GPU storage location of u_ModelMatrix uniform');
    return;
  }
  u_VBOnumLoc = gl.getUniformLocation(g_ShaderID1, 'u_VBOnum');
  if (!u_VBOnumLoc) {
...
// Create a buffer object in the graphics hardware: save its ID# in global var: 
 ...
 // 			times and then discarded.
  ...
  a_PositionLoc = gl.getAttribLocation(g_ShaderID1, 'a_Position');
  if(a_PositionLoc < 0) {
  ...
  // Next attribute: a) get the GPU location...
 	a_ColorLoc = gl.getAttribLocation(g_ShaderID1, 'a_Color');
  if(a_ColorLoc < 0) {
