...
/* Demonstrate use of two separate VBOs with different contents & attributes. 
	...
	CONVERSION:
	11a)--create a vboBox1 object named 'leftView' and, one-by-one, comment out 	
		each existing vbo1-related VARIABLE and replace w/ 'leftView' member; test. 
	11b)--comment out each vbo1-related FUNCTION call and function, and replace 
	with 'leftView' members (all of initVBO1(), parts of draw(), etc) test;
...
//	(see OpenGL ES specification for more info).  Your choices are:
 ...
 // 			times and then discarded; for rapidly supplied & consumed VBOs.
  myGL.bufferData(gl.ARRAY_BUFFER, 			// GLenum target(same as 'bindBuffer()')
 					 leftView.vboContents, 				// JavaScript Float32Array
...
leftView = new VBObox1();		// Holds one VBO and its shaders.
...
var vbo1Array = new Float32Array ([		// Array of vertex attributes we will 
														...
														// 1 vertex per line: pos x,y,z,w; color: r,g,b 
	//1st triangle:
...
//var g_vboID1;										// 1st Vertex Buffer ObjectID#, created by GPU
//var g_ShaderID1;								// Shader-program ID# created in main() by GPU
//																// for V,F shaders that draw VBO1 contents
...
var g_vboID2;											// ID# for 2nd VBO.
var g_ShaderID2;									// Shader-program ID# created in main() by GPU
//																// for V,F shaders that draw VBO2 contents
...
var a_PtSizeLoc2;							// GPU location for 'a_PtSize' attrib in Shader 2)

//-----uniforms	in GPU:
//var g_ModelMatrix = new Matrix4();// Transforms CVV drawing axesin to 'model' axes
...
//=============================================================================
  ...
  leftView.init(gl);			// VBO + shaders + uniforms + attribs for WebGL preview
...
//  leftView.shaderLoc = createProgram(gl, leftView.VERT_SRC, leftView.FRAG_SRC);	// for VBO1,
  g_ShaderID2 = createProgram(gl, VSHADER_2SRC, FSHADER_2SRC);	// for VBO2.
/*  
  if (!leftView.shaderLoc || !g_ShaderID2) {
...
/*
  ...
  if (n < 0) {
    console.log('Failed to make FIRST vertex buffer object (VBO) in the GPU!');
    return;
  }
*/
	n = initVBO2();					// init the 2nd VGO: returns # of vertices created.
	if (n < 0) {
		console.log('Failed to make SECOND vertex buffer object(VBO) in the GPU!');
	}
...
/*
 leftView.vboLoc = gl.createBuffer();	
...
/*
  ...
  gl.bindBuffer(gl.ARRAY_BUFFER,				// GLenum 'target' for this GPU buffer 
  								leftView.vboLoc);			// the ID# the GPU uses for this buffer.
  											
 // Transfer data from our JavaScript Float32Array object to the GPU, into the 
 // VBO (a named hunk of memory) we just created, identified by leftView.vboLoc. 
 ...
 // 			times and then discarded.
  gl.bufferData(gl.ARRAY_BUFFER, 				// GLenum target(same as 'bindBuffer()')
 					 leftView.vboContents, 				// data source: JavaScript Float32Array
  							 gl.STATIC_DRAW);				// Usage hint.
*/
/*
// Get GPU storage location for each uniform var used in the shader program 
// found at leftView.shaderLoc: 
// Version 9b):  MOVE THIS to initVBO1() and initVBO2()
 leftView.u_ModelMatLoc = gl.getUniformLocation(leftView.shaderLoc, 'u_ModelMat1');
  if (!leftView.u_ModelMatLoc) { 
...
/*
  ...
  leftView.a_PosLoc = gl.getAttribLocation(leftView.shaderLoc, 'a_Pos1');
  if(leftView.a_PosLoc < 0) {
  ...
  gl.vertexAttribPointer(
		leftView.a_PosLoc,//index == ID# for the attribute var in your GLSL shaders;
		4,						// size == how many dimensions for this attribute: 1,2,3 or 4?
		gl.FLOAT,			// type == what data type did we use for those numbers?
		false,				// isNormalized == are these fixed-point values that we need
									//									normalize before use? true or false
		7*leftView.FSIZE,	// stride == #bytes we must skip in the VBO to move from one 
									// of our stored attributes to the next.  This is usually the 
									// number of bytes used to store one complete vertex.  If set 
  ...
  gl.enableVertexAttribArray(leftView.a_PosLoc);
*/
  //------------------------
/*
  // Next attribute: a) get the GPU location..
 	leftView.a_ColrLoc = gl.getAttribLocation(leftView.shaderLoc, 'a_Colr1');
  if(leftView.a_ColrLoc < 0) {
    console.log('initVBO1() failed to get the GPU storage location of a_Colr1');
    return -1;
  }

  // b) tells GPU shader 1 how to retrieve attrib from currently bound buffer:
  gl.vertexAttribPointer(leftView.a_ColrLoc, 3, gl.FLOAT, false, 7*leftView.FSIZE, 4*leftView.FSIZE);
  // c) Enable this assignment of a_Colr1 attribute to the bound buffer:
  gl.enableVertexAttribArray(leftView.a_ColrLoc);
*/
  return leftView.vboVerts;
}
...
//=============================================================================
 g_vboID2 = gl.createBuffer();	
  ...
  if (!g_vboID2) {
    console.log('initVBO2() failed: no 2nd vertex buffer object(VBO) in GPU');
    return -1;
  }
  // "Bind the new buffer object (memory in the graphics system) to target"
  // In other words, specify the usage of this selected buffer object.
  ...
  //	== "gl.ELEMENT_ARRAY_BUFFER" meaning the buffer object holds indices 
  // 			into a list of attributes needed; indices such as object #s, face #s, 
  //			edge vertex #s.
  gl.bindBuffer(gl.ARRAY_BUFFER,				// GLenum 'target' for this GPU buffer 
  											g_vboID2);			// the ID# the GPU uses for this buffer.
 // Transfer data from our JavaScript Float32Array object to the GPU, into the 
 // VBO (a named hunk of memory) we just created, identified by g_vboID2. 
 //  Recall that gl.bufferData() allocates and fills a *NEW* hunk of graphics 
 //		memory.  We always use gl.bufferData() in the creation of a new buffer.T
 //	The 'hint' we give helps the GPU manage memory for speed and efficiency,
 // and we selected it by following the OpenGL ES specification:
...
// Version 9b):  MOVE THIS to initVBO1() and initVBO2()
 ...
 u_ModelMatrixLoc2 = gl.getUniformLocation(g_ShaderID2, 'u_ModelMatrix');
  ...
  if(a_ColorLoc2 < 0) {
    console.log('initVBO2() failed to get GPU storage location2 of a_Color');
    return -1;
  }
...
//=============================================================================
  ...
  gl.clear(gl.COLOR_BUFFER_BIT);
	...
	if(g_show1 == 1) {	// IF user didn't press HTML button to 'hide' VBO1:
		leftView.adjust(gl);		// Send new values for uniforms to the GPU, and
		leftView.draw(gl);			// draw our VBO's contents using our shaders.
/*
	  // DRAW FIRST VBO:--------------------------
	  gl.useProgram(leftView.shaderLoc);	// In the GPU, SELECT one of the vertex 
	  																		// and shader pgms created, compiled, 
	  																		// and linked in main().
	  leftView.ModelMat.setRotate(g_currentAngle, 0, 0, 1);	// rotate drawing axes,
	  leftView.ModelMat.translate(0.35, 0, 0);		// then translate them.
	  //  Transfer 'ModelMatrix' values to the GPU's 'u_ModelMat1' uniform: 
	  gl.uniformMatrix4fv(leftView.u_ModelMatLoc, // GPU location of the uniform
	  										false, 									// use matrix transpose instead?
	  										leftView.ModelMat.elements);// JS data to send to GPU

		// CHANGE BUFFER BINDING TO VBO1------------ (SURPRISE! Not enough!)
		gl.bindBuffer(gl.ARRAY_BUFFER,			// GLenum 'target' for this GPU buffer 
  								leftView.vboLoc);			// the ID# the GPU uses for this buffer.
  	// **** SURPRISE!  changing the buffer binding DOESN'T change the data 
  	// source for the 'attributes' in our shaders! to do that, we must call
  	// gl.vertex AttribPointer() to tie shader's 'a_Pos1' attribute to the 
  	// currently-bound buffer:
		// (Here's how to use the almost-identical OpenGL version of this function:
		//		http://www.opengl.org/sdk/docs/man/xhtml/glVertexAttribPointer.xml )
	  // ----------Tie shader's 'a_Pos1' attribute to bound buffer:------------
	  gl.vertexAttribPointer(leftView.a_PosLoc, 4, gl.FLOAT, false, 7*leftView.FSIZE, 0);
	  ...
	  gl.drawArrays(gl.TRIANGLES, 				// select the drawing primitive to draw,
	  							0, 										// location of 1st vertex to draw;
	  							leftView.vboVerts);		// number of vertices to draw on-screen.
*/
  }
  if(g_show2 == 1) { // IF user didn't press HTML button to 'hide' VBO2:
	  ...
	  gl.useProgram(g_ShaderID2);					// In the GPU, SELECT one of the vertex 
	  																		// and shader pgms created, compiled, 
	  																		// and linked in main().
	  leftView.ModelMat.setRotate(-g_currentAngle, 0, 0, 1);	// -spin drawing axes,
	  leftView.ModelMat.translate(0.55, 0, 0);				// then translate them further:
	  //  Transfer 'leftView.ModelMat' values to the GPU's 'u_ModelMatrix' uniform: 
	  gl.uniformMatrix4fv(u_ModelMatrixLoc2, 			// GPU location of the uniform
	  										false, 									// use matrix transpose instead?
	  										leftView.ModelMat.elements);// Javascript data to send GPU
