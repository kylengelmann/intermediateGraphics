...
/* Demonstrate use of two separate VBOs with different contents & attributes. 
	...
	Clues:
		...
		08)--EASY HACK WAY:  When we switch from rendering one VBO to another, use 
			...
			the shader won't use a_PtSize values it might retrieve from VBO1.  We only 
	...
	09:--BETTER, PROPER WAY:==================================================
		Let's create two different shader programs; a vertex shader and fragment 
		shader for VBO1, and a different vertex shader and fragment shader for 
		VBO2.  We will follow "WebGL Programming Guide" textbook, Chap 10, pg. 386 
		example code: "ProgramObject.js".
	09a)--Rename shaders (VSHADER_1SRC, FSHADER_1SRC) and test; then make and 
			test copies of them named (VSHADER_2SRC, FSHADER_2SRC).
		  --Modify 2nd shaders to look different; to blend colors w/ 50% purple.
	09b)--In main(), replace the 'initShaders()' call with createProgram() calls 
		(both found in textbook's 'cuon-utils.js' library) for the two shaders.
			--First, use g_ShaderID1 program ONLY: replace every instance of 
			gl.program (e.g. args to getAttribLocation(), getUniformLocation(), etc) 
			with  g_ShaderID1, and 'useProgram(g_ShaderID1)' at the start of draw().
		------------------
		Good! now it's time to UNTANGLE the shaders, VBOs,uniforms, & attributes.
		We want Shader1 to draw ONLY the contents of VBO1 using ONLY attributes that 
		always get data ONLY from VBO1.  Similarly, 
		we want Shader2 to draw ONLY the contents of VBO2 using ONLY attributes that 
		always get data ONLY from VBO2. 
		-----------------
	09c)--STOP sharing the same uniforms and attributes for different VBOs. First, 
	re-name attributes a_Position, a_Color, a_PtSize and their locations to
	a_Position1, a_Color1, and a_PtSize1 (a_Position1Loc,a_Color1Loc,aPtSize1Loc) 
	and test. Next, re-name uniforms u_ModelMatrix, u_VBOnum and their locations
	to u_ModelMatrix1, u_VBOnum1 (u_ModelMatrix1Loc, u_VBOnum1Loc).  Test.
	--Next, create a new set of attributes a_Position2, a_Color2, a_PtSize2 that
	we will use ONLY for VBO2.   
	 for use with VBO2 ONLY (note they're still used in both shaders, though!)
*/
...
var VSHADER_1SRC =										// VERTEX SHADER for VBO1
  ...
  'attribute float a_PtSize1; \n' +
  'varying vec3 v_Colr;\n' +
  'void main() {\n' +
  '  if(u_VBOnum1==2) {\n' +
  '    gl_PointSize = a_PtSize1;\n' +
  '		 } \n' +
  '		else {gl_PointSize = 1.0;}\n' +
  '  gl_Position = u_ModelMatrix1 * a_Position1;\n' +
  '	 v_Colr = a_Color1;\n' +
  ' }\n';
  
var VSHADER_2SRC =										// VERTEX SHADER for VBO2: reddish tinged
  'precision highp float;\n' +				// req'd in OpenGL ES if we use 'float'
  //
  'uniform mat4 u_ModelMatrix1;\n' +
	'uniform  int u_VBOnum1;\n' + 
  ...
  '		else {gl_PointSize = 1.0;}\n' +
  '  gl_Position = u_ModelMatrix1 * a_Position1;\n' +
  '	 v_Colr = a_Color1*0.6 + vec3(0.4, 0.0, 0.0);\n' + // ALL 40% red color!
  ' }\n';
...
var FSHADER_2SRC =									// FRAGMENT SHADER for VBO2: bluish tinged
  ...
  'varying vec3 v_Colr;\n' +
  'void main() {\n' +
  '  gl_FragColor = vec4(v_Colr[0], v_Colr[1], 1.0, 1.0);\n' +  
...
var a_Position1Loc;							// GPU location for 'a_Position1' attrib in VBO1 or VBO2
var a_Color1Loc;								// GPU location for 'a_Color1' attrib in VBO1 or VBO2
var a_PtSize1Loc;							// GPU locatin for 'a_PtSize1' attrib in VBO2 only.
//-----uniforms	in GPU:
var g_ModelMatrix = new Matrix4();// Transforms CVV drawing axes to 'model' axes.
var u_ModelMatrix1Loc;						// GPU storage location for u_ModelMatrix1 uniform
var u_VBOnum1Loc;								// GPU storage location for u_VBOnum1 uniform.
...
//==============================================================================
  ...
  if (!g_ShaderID1 || !g_ShaderID2) {
    console.log('Failed to intialize GLSL shaders. Bye!');
    return;
  }
...
// Get GPU storage location for all uniform vars used by any of our shaders:
  u_ModelMatrix1Loc = gl.getUniformLocation(g_ShaderID1, 'u_ModelMatrix1');
  if (!u_ModelMatrix1Loc) { 
    console.log('Failed to get GPU storage location of u_ModelMatrix1 uniform');
    return;
  }
  u_VBOnum1Loc = gl.getUniformLocation(g_ShaderID1, 'u_VBOnum1');
  if (!u_VBOnum1Loc) {
		console.log('Failed to get GPU storage location of u_VBOnum1 uniform');
		return;
  }
  // ==============ANIMATION=============
...
// Create a buffer object in the graphics hardware: save its ID# in global var: 
 ...
 // 			times and then discarded.
  gl.bufferData(gl.ARRAY_BUFFER, 				// GLenum target (same as 'bindBuffer()')
	...
	// ----------Tie shader's 'a_Position1' attribute to bound buffer:-------------
  // a) Get the GPU location for the a_Position1 var in the graphics hardware:
  a_Position1Loc = gl.getAttribLocation(g_ShaderID1, 'a_Position1');
  if(a_Position1Loc < 0) {
    console.log('initVBO1() Failed to get the GPU storage location of a_Position1');
    return -1;
  }
  // b) Next, set GPU to fill 'a_Position1' attribute variable for vertex with 
  // values stored in the buffer object chosen by 'gl.bindBuffer()' command.
	// (Here's how to use the almost-identical OpenGL version of this function:
	//		http://www.opengl.org/sdk/docs/man/xhtml/glVertexAttribPointer.xml )
  gl.vertexAttribPointer(
		a_Position1Loc,//index == ID# for the attribute var in your GLSL shaders;
  ...
  // c) Enable this assignment of the a_Position1 attribute to the bound buffer:
  gl.enableVertexAttribArray(a_Position1Loc);
  //------------------------
  // Next attribute: a) get the GPU location...
 	a_Color1Loc = gl.getAttribLocation(g_ShaderID1, 'a_Color1');
  if(a_Color1Loc < 0) {
    console.log('initVBO1() failed to get the GPU storage location of a_Color1');
    return -1;
  }
  // b) tell GPU how to retrieve attrib from the currently bound buffer:
  gl.vertexAttribPointer(a_Color1Loc, 3, gl.FLOAT, false, 7*FSIZE1, 4*FSIZE1);
  // c) Enable this assignment of a_Color1 attribute to the bound buffer:
  gl.enableVertexAttribArray(a_Color1Loc);
  //--------------------------------------------
  // Next attribute: a) get the GPU location...
  // !NEW! 'a_PtSize1' attribute values are stored only in VBO2, not VBO1:
  a_PtSize1Loc = gl.getAttribLocation(g_ShaderID1, 'a_PtSize1');
  if(!a_PtSize1Loc) {
		console.log('initVBO1() failed to get GPU location of a_PtSize1 attribute');
  }
  // b) tell GPU how to retrieve attrib from currently bound buffer (VBO1):
  gl.vertexAttribPointer(a_PtSize1Loc, 1, gl.FLOAT, false, 
...
//==============================================================================
 ...
 // 			times and then discarded.
  ...
  a_Position1Loc = gl.getAttribLocation(g_ShaderID1, 'a_Position1');
  if(a_Position1Loc < 0) {
    console.log('initVBO2() failed to get the GPU location of a_Position1');
    return -1;
  }
  // b) Next, set GPU to fill 'a_Position1' attribute variable for vertex with 
  // values stored in the buffer object chosen by 'gl.bindBuffer()' command.
  ...
  // Next attribute: a) get the GPU location...
 	a_Color1Loc = gl.getAttribLocation(g_ShaderID1, 'a_Color1');
  if(a_Color1Loc < 0) {
    console.log('initVBO2() failed to get the GPU storage location of a_Color1');
    return -1;
  }
  // b) tell GPU how to retrieve attrib from the currently bound buffer:
  gl.vertexAttribPointer(a_Color1Loc, 3, gl.FLOAT, false, 7*FSIZE2, 4*FSIZE2);
  // c) Enable this assignment of a_Color1 attribute to the bound buffer:
  gl.enableVertexAttribArray(a_Color1Loc);
  //--------------------------------------------
  // Next attribute: a) get the GPU location...
  ...
  a_PtSize1Loc = gl.getAttribLocation(g_ShaderID1, 'a_PtSize1');
  if(!a_PtSize1Loc) {
		console.log('initVBO2() failed to get GPU location of a_PtSize1 attribute');
  }
...
//==============================================================================
  ...
  gl.clear(gl.COLOR_BUFFER_BIT);
	...
	if(g_show1 == 1) {	// IF user didn't press HTML button to 'hide' VBO1:
	  ...
	  gl.useProgram(g_ShaderID1);					// In the GPU, SELECT one of the vertex 
	  																		...
	  																		// and linked in main().
		gl.uniform1i(u_VBOnum1Loc, 1);										// set uniform value to 1
	  g_ModelMatrix.setRotate(g_currentAngle, 0, 0, 1);	// rotate drawing axes,
	  g_ModelMatrix.translate(0.35, 0, 0);						// then translate them.
	  //  Transfer 'g_NodelMatrix' values to the GPU's 'u_ModelMatrix1' uniform: 
	  gl.uniformMatrix4fv(u_ModelMatrix1Loc, 			// GPU location of the uniform
  	...
  	// gl.vertex AttribPointer() to tie shader's 'a_Position1' attribute to the 
  	// currently-bound buffer:
		// (Here's how to use the almost-identical OpenGL version of this function:
		//		http://www.opengl.org/sdk/docs/man/xhtml/glVertexAttribPointer.xml )
	  // ---------Tie shader's 'a_Position1' attribute to bound buffer:------------
	  	  gl.vertexAttribPointer( a_Position1Loc, 4,gl.FLOAT,	false, 7*FSIZE1, 0);
	  // enable the newly-re-assigned a_Position1 attribute:
	  gl.enableVertexAttribArray(a_Position1Loc);
	  // ----------Tie shader's 'a_Color1' attribute to bound buffer:--------------
	  gl.vertexAttribPointer(a_Color1Loc, 3, gl.FLOAT, false, 7*FSIZE1, 4*FSIZE1);
	  // then enable this assignment of a_Color1 attribute to the bound buffer:
	  gl.enableVertexAttribArray(a_Color1Loc);
	  //-----------Tie shader's 'a_PtSize1' attribute to bound buffer:-------------
	  gl.vertexAttribPointer(a_PtSize1Loc, 1, gl.FLOAT, false, 7*FSIZE2, 0);
	  // (VBO1 doesn't hold any a_PtSize1 values, so we just use the 1st float
	  // found in each vertex.  Shader will access this value, but not use it).
	  // Enable this assignment of a_PtSize1 attribute to the bound buffer:
	  gl.enableVertexAttribArray(a_PtSize1Loc);
  ...
  if(g_show2 == 1) { // IF user didn't press HTML button to 'hide' VBO2:
	  ...
	  gl.useProgram(g_ShaderID1);					// In the GPU, SELECT one of the vertex 
	  																		// and shader pgms created, compiled, 
	  																		// and linked in main().
		gl.uniform1i(u_VBOnum1Loc, 2);										// set uniform value to 2
	  ...
	  //  Transfer 'g_ModelMatrix' values to the GPU's 'u_ModelMatrix1' uniform: 
	  gl.uniformMatrix4fv(u_ModelMatrix1Loc, 			// GPU location of the uniform
	  										false, 									// use matrix transpose instead?
	  										g_ModelMatrix.elements);// Javascript data to send to GPU
		...
		gl.bindBuffer(gl.ARRAY_BUFFER,		// GLenum 'target' for this GPU buffer 
	  											g_vboID2);		// the ID# the GPU uses for this buffer.
	  // ---------Tie shader's 'a_Position1' attribute to bound buffer:------------
	  gl.vertexAttribPointer( a_Position1Loc, 4,	gl.FLOAT,	false, 8*FSIZE2, 0);
	  // Enable this assignment of the a_Position1 attribute to the bound buffer:
	  gl.enableVertexAttribArray(a_Position1Loc);
	  // ----------Tie shader's 'a_Color1' attribute to bound buffer:--------------
	  gl.vertexAttribPointer(a_Color1Loc, 3, gl.FLOAT, false, 8*FSIZE2, 4*FSIZE2);
	  // Enable this assignment of a_Color1 attribute to the bound buffer:
	  gl.enableVertexAttribArray(a_Color1Loc);
	  //-----------Tie shader's 'a_PtSize1' attribute to bound buffer:-------------
	  gl.vertexAttribPointer(a_PtSize1Loc, 1, gl.FLOAT, false, 8*FSIZE2, 7*FSIZE2);
	  // Enable this assignment of a_PtSize1 attribute to the bound buffer:
	  gl.enableVertexAttribArray(a_PtSize1Loc);
		// ****** END SURPRISE.
