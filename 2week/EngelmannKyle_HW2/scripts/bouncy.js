...
function main() {
  ...
  partSys = new particleSystem(numP, VSHADER_SOURCE, FSHADER_SOURCE);

  for(var i = 0; i < numP; i++) {
    partSys.s0.setParticlePosition(i, startPosition);
    var vec = roundRand3D();
    partSys.s0.setParticleVelocity(i, [vec[0]*velKick, vec[1]*velKick, vec[2]*velKick]);
    // partSys.setParticleMass(0, 1);
    partSys.s0.setParticleColor(i, [(vec[0]+1)*.5, (vec[1]+1)*.5, (vec[2]+1)*.5, 1]);
    partSys.s0.setParticleSize(i, 30);
    partSys.s0.setParticleForce(i, [0, 0, gravity*partSys.s0.getParticleMass(0)]);
  }
  ...
  constraints.push(fwall);
  constraints.push(bwall);
  constraints.push(rwall);
  constraints.push(lwall);

  var gravity = new constantAcceleration([0, 0, -9.8]);
  var drag = new dragForce(1);
  ...
  partSys.init(gl, partAttr, partAttrOffsets, partAttrSizes, partUnifs, 
                gl.ARRAY_BUFFER, gl.DYNAMIC_DRAW, gl.POINTS);
  ...
  var tick = function() {
    ...
    if(Keys.keyboard['r'].down) {
      for(var i = 0; i < numP; i++) {
        ...
        if(Keys.keyboard["shift_l"].pressed) {
          ...
          var vel = partSys.s0.getParticleVelocity(i);
          partSys.s0.setParticleVelocity(i, [vel[0] + vec[0]*velKick, vel[0] + vec[1]*velKick, vel[0] + vec[2]*velKick]);
        }
      }
    }
    ...
    if(!singleStep || takeStep) {
      partSys.applyForces(forces);
      partSys.update(elapsed);
      partSys.applyConstraints(constraints);
      takeStep = false;
    }
