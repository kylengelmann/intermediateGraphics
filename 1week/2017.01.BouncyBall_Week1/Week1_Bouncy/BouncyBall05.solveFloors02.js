...
//		--add 'stepCount' to give each timestep a unique label for debugging;
		...
		if(g_bounce==0) { //--------------------------------------------------------
			...
			if(      xposNow < 0.0 && xvelNow < 0.0 // collision!  left wall...
				...
				xposNow = 0.0;					// 1) resolve contact: put particle at wall.
																...
																// velocity gained from forces applied while the // ball moved thru wall during this timestep. HOW?  
				...
				xvelNow = xvelPrev;			// we had a the START of the timestep.
				...
				// 		Unwittingly, we already created that result!
				...
				// gravity acting constant mass, plus constant drag force, etc).  If the ball 'bounces' (reverses velocity) exactly halfway through the timestep, at the statistical 'expected value' for collision time, then the constant force that acts to INCREASE velocity in one half-timestep will DECREASE it in the other half timestep by exactly the same amount -- and thus removes ALL the velocity added during the timestep.)
				xvelNow *= g_drag;			// **BUT** velocity during our timestep is STILL 
				...
				// floor) can easily from positive to negative between 'previous' and 
				// 'now' timesteps.  Put another way:
				// Step 2) CHANGED the 'now' ball velocity, and may have changed its 
				// sign.  We need a sign test that ensures the 'bounce' step will always 
				// send the ball outwards, away from its wall or floor collision. 
