...
function main() {
  ...
  var fixed = new fixedConstraint([0], [[0, 0, numP-2]]);
  ...
  for(var i = 1; i < numP; i++) {
    var spring = new springForce(1, 50, [i-1, i]);
    springs.push(spring);
    forces.push(spring);
  }
  ...
  for(var i = 0; i < numP; i++) {
    partSys.Particles[i].setPosition([i, 0, numP-2]);
    var vec = roundRand3D();
    partSys.Particles[i].setColor([(vec[0]+1)*.5, (vec[1]+1)*.5, (vec[2]+1)*.5, 1]);
    partSys.Particles[i].setSize(50);
    partSys.Particles[i].setMass(1);
  }
  ...
  var sAttr = ["a_Position"];
  var sAttrSizes = [3];
  var sAttrOffsets = [PARTICLE_POS];
  var sUnifs = ["u_ModelMatrix", "u_ViewMatrix", "u_ProjMatrix", "u_Color"];
  ...
  partSys.springRenderer.setVec4(gl, "u_Color", [0, 1, 1, 1]);
  ...
  var tick = function() {
    ...
    if(Keys.keyboard['r'].pressed) {
      for(var i = 0; i < numP; i++) {
        ...
        partSys.Particles[i].setColor([(vec[0]+1)*.5, (vec[1]+1)*.5, (vec[2]+1)*.5, 1]);
        partSys.Particles[i].setSize(50);
        partSys.Particles[i].setVelocity([0, 0, 0])
      }
    }
    if(Keys.keyboard['up_arrow'].pressed) {
      var vel = partSys.Particles[numP-1].getVelocity();
      partSys.Particles[numP-1].setVelocity([vel[0], vel[1], vel[2] + velKick]);
    }
    ...
    if(Keys.keyboard['right_arrow'].pressed) {
      var vel = partSys.Particles[numP-1].getVelocity();
      partSys.Particles[numP-1].setVelocity([vel[0] + mainCam.right[0]*velKick, vel[1] + mainCam.right[1]*velKick, vel[2]]);
    }
